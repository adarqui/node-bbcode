...
exports.parse = function(post, cb) {
  ...
  var tagname_re = /^\/?(?:b|i|u|pre|samp|code|colou?r|size|noparse|url|link|s|q|blockquote|quote|img|u?list|li|hr|youtube)$/i;
  ...
  function textToHtmlCB(mstr, m1, m2, m3, m4, offset, string)
  {
    ...
    if(isValidTag(m2)) {
      ...
      switch (m2l) {
		...
		case "hr":
			opentags.push(new taginfo_t('hr', '</hr>'))
			return '<hr>';

		case "youtube":
         opentags.push(new taginfo_t(m2l, "</iframe>"));

          // check if there's a valid option
          if(m3 && uri_re.test(m3)) {
            // if there is, output a complete start anchor tag
            urlstart = -1;
            return "<iframe width=\"420\" height=\"315\" src=\"" + m3 + "\" frameborder=\"0\" allowfullscreen>";
          }
          urlstart = mstr.length + offset;
			return "<iframe width=\"420\" height=\"315\" frameborder=\"0\" allowfullscreen src=\"";
